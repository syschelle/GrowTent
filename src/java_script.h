...
function updatePumpAvailabilityFromFlags(flags) {
    ...
    pumpRelays.forEach((relayNo, index) => {
        ...
        if (button) {
            ...
            button.title = isEnabled ? '' : (I18N['status.pumpDisabledHint'] || 'Pump disabled');
        }

        // Keep disabled pumps visible while making their inactive state unmistakable.
        if (card) {
...
function clampInt(x, lo, hi){
  ...
  function applyTranslations(){
    ...
    if (window._lastApiState) {
      updatePumpAvailabilityFromState(window._lastApiState);
    }
  }
